...
- [x] KAMIS Open API 신청 → cert_key + cert_id(`7790`) 발급 수령
...
- [x] cert_id = `7790` (숫자 ID) 확인 — 이메일 아님
...
1. **Phase 7: Dockerfile 작성 → Railway 배포** ← 즉시 가능
2. 배포 완료 후 데이터 백필 (지난 30일치 수동 수집)
3. 데이터 7일 쌓이면 Chart.js 변동 그래프 추가
...
### 2026-04-28
- Phase 2 완료: API 키 수령(cert_id=7790), SSL 해결, 실제 호출 성공, 코드 매핑 완료
