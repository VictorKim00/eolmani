...
- **마지막 업데이트**: 2026-04-30 (품목 42→59종 확장 + Phase 22 지역별 가격 지원 완료)
...
**Phase 22 완료 · 품목 59종 · 다음: 품질 개선 + 공산품 섹션**:
지역별 가격 지원(24개 도시), 품목 42→59종, 그래프 개선, 날씨 지역화 완료.
다음: UX 소규모 개선(U3·U5) 또는 Week 4~5 공산품 섹션 착수.
...
UX 현대화 + 품목 확장       ██████████  100%  ✅ 59종, 그룹 시스템, 탭 필터
...
1. **U3: 절약 금액 표시** — 🟢 신호 시 "평년 대비 X원 절약" 표시 (`item_detail.html`)
2. **U5: 마지막 수집 시각 표시** — 헤더 또는 푸터에 "오늘 XX:XX 기준" 라벨
3. **배추/무 계절 변형** — 봄배추(kind=01) 5월부터 데이터 나옴. cucumber 그룹처럼 변형 탭 추가
4. **Week 4~5: 공산품 섹션** — 참가격 API 연동, `/commodities` 라우트, 마트별 비교 카드
5. **포도(414)/복숭아(413) 시즌 품목** — 6~9월 시즌 확인 후 seed 추가
