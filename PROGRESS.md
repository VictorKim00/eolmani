...
- **시작일**: 2026-04-27
- **현재 주차**: Week 3 완료 + Week 4 진입 준비
- **마지막 업데이트**: 2026-04-30 (UX 현대화 + 그룹 시스템 + 탭 필터 + 검색 완료)
...
**UX 현대화 완료 · 다음: 지역별 가격 지원 + 여름 품목 확장**:
품목 20→42종, 그룹/변형 시스템, 카드 리디자인, 탭 필터+검색 완료.
다음 큰 기능: 지역별 가격 지원 (현재 서울 고정 → 사용자 지역 선택).
...
UX 현대화 + 품목 확장       ██████████  100%  ✅ 42종, 그룹 시스템, 탭 필터
Phase 22 지역별 가격         ░░░░░░░░░░    0%  ⬜ 다음 목표
Week 4~5 (공산품)           ░░░░░░░░░░    0%
보류 (가격 알림)            ░░░░░░░░░░    0%
보류 (영수증 OCR)           ░░░░░░░░░░    0%
보류 (개인 CPI/슈링크)      ░░░░░░░░░░    0%
...
## 🗂 KAMIS 코드 — 확정된 품목 42종 (2026-04-30 기준, 실제 검증)
...
| cabbage | 배추 | — | 211 | 06 | 상품 | 1포기 |
| radish | 무 | — | 231 | 06 | 상품 | 1개 |
...
## ⬜ Phase 22 — 지역별 가격 지원 (다음 목표)

> 현재 서울(1101) 고정 → 사용자가 자기 지역 가격을 볼 수 있도록.
> KAMIS API는 `p_country_code` 파라미터로 지역별 가격 제공. 지역마다 가격 차이가 큼.

### 배경 & 중요성

- 현재 모든 가격은 서울(1101) 소매가 기준 — 지방 사용자는 체감 괴리 있음
- KAMIS 지역별 코드: 서울(1101), 부산(2100), 대구(2200), 인천(2300), 광주(2401), 대전(2501), 울산(2601), 세종(3611), 경기(3111), 강원(3211), 충북(3311), 충남(3411), 전북(3511), 전남(3611), 경북(3711), 경남(3811), 제주(3911)
- 1차: 주요 6대 도시 (서울, 부산, 대구, 인천, 광주, 대전) — API 부담 적음
- 사용자 체감: "내 지역 기준으로 봤더니 서울보다 XX원 더 비싸네" → 실용적 가치 ↑

### 구현 계획

- [ ] **DB 스키마**: `price_history`에 `region_code VARCHAR(10)` 컬럼 추가 (Alembic 마이그레이션)
  - 기존 데이터는 `region_code='1101'` (서울)로 backfill
  - `(item_id, recorded_date, source, region_code)` Unique constraint 조정
- [ ] **수집 확장** (`price_collector.py`): 6개 지역 × 현재 API 호출 수
  - 지역 순회 수집, 실패 시 해당 지역만 건너뜀 (다른 지역 영향 없음)
  - KAMIS 수집 시간 06:00 KST, 지역별 병렬 or 순차 선택
- [ ] **price_service.py**: `region_code` 파라미터 추가, 기본값 `'1101'`
- [ ] **설정**: `COLLECTED_REGIONS = ['1101', '2100', '2200', '2300', '2401', '2501']`
- [ ] **UI 지역 선택기**: 헤더에 지역 드롭다운 (localStorage 저장)
  - 첫 방문 시 기본값 서울, 언제든 변경 가능
- [ ] **지역 비교 표시**: 상세 페이지에 "서울보다 X% 비쌈/저렴" 뱃지 (선택)
- [ ] **백필**: 신규 지역 수집 시작 시점부터 데이터 축적 (과거 역주행 불필요)

### 난이도 & 주의사항

- API 호출 6배 증가 → KAMIS 초당 요청 제한 확인 필요 (현재 무제한인 듯)
- PriceHistory Unique constraint 변경 시 기존 데이터 마이그레이션 신중히
- 지역별 가격 없는 날 → 서울 폴백 or `None` 처리 필요
...
1. **Phase 22: 지역별 가격 지원** — PriceHistory에 region_code 추가, 6대 도시 수집, 헤더 지역 선택기 UI
2. **여름 품목 추가** — 포도(414), 복숭아(413), 수박(221) 5~9월 시즌 품목. KAMIS API로 현재 데이터 확인 후 seed 추가
3. **배추/무 계절 변형** — 봄배추(kind=01) 5월부터 데이터 나옴. cucumber 그룹처럼 spring/여름 변형 추가
4. **U4: 차트 기간 탭** — 품목 상세 페이지에 30일/90일/1년 탭 전환 (Chart.js 데이터 필터링)
5. **수산 품목 추가** — 건멸치(638), 김(641), 새우(631) 확인 후 추가
