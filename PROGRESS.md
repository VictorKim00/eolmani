...
- **시작일**: 2026-04-27
- **현재 주차**: 친구 피드백 Sprint 2 진행 중
- **마지막 업데이트**: 2026-05-05 (Sprint 2 — 쿠팡 파트너스 링크 25개 등록 + 공정위 고지문구 완료)
...
**Sprint 2 마무리 단계**:
쿠팡 파트너스 링크 25개 등록 완료 (쌀·감자·배추·양파·대파·마늘·사과·계란·삼겹살·고등어 등).
공정위 심사지침 준수 고지 문구 상세 페이지 상단에 배치 완료.
다음: 전단지형 히어로 카드 (메인 최상단 🟢 품목 강조)
...
친구 피드백 Sprint 1        ██████████  100%  ✅ 단위·기준·글씨·정렬
친구 피드백 Sprint 2        █████████░   90%  🛒 파트너스 링크 25개 등록 · 공정위 고지 완료 · 히어로 카드 남음
SEO + 애널리틱스            ████████░░   80%  ✅ 메타태그·sitemap·robots·GA4(G-1XQ2HKQE3M)·서치콘솔 완료 · 네이버 웹마스터 남음
Week 4~5 (공산품)           ░░░░░░░░░░    0%
보류 (가격 알림)            ░░░░░░░░░░    0%
보류 (영수증 OCR)           ░░░░░░░░░░    0%
보류 (개인 CPI/슈링크)      ░░░░░░░░░░    0%
...
1. **Sprint 2 히어로 카드** — 메인 최상단 🟢 신호 품목 1~3개 전단지형 강조 카드
2. **쿠팡 파트너스 링크 추가 등록** — 오이·호박·상추·참외·수박·한우·목심·오징어 등 나머지 품목 `scripts/set_coupang_urls.py` 채우기 → Railway DB 적용
3. **네이버 웹마스터도구 등록** — https://searchadvisor.naver.com → 사이트 등록 → sitemap 제출
4. **커뮤니티 홍보** — 에펨코리아·클리앙·82cook·맘카페 타겟별 포스팅. 5월 = 수박/참외/마늘 시즌 훅 활용
5. **Week 4~5: 공산품 섹션** — 참가격 API 연동, `/commodities` 라우트, 마트별 비교 카드
6. **U2: grey 신호등** — 데이터 없을 때 "보통" 오해 방지 (grey 처리)
...
### 2026-05-05 — 쿠팡 파트너스 링크 25개 등록 + 공정위 고지 문구 위치 수정

**쿠팡 파트너스 링크 활성화**
- `set_coupang_urls.py` 25개 품목 URL 주석 해제 (쌀·쌀10kg·감자·고구마·배추·무·양파·대파·마늘·시금치·토마토·당근·사과·배·딸기·바나나·수박·삼겹살·닭고기·계란·고등어·갈치·김)
- 계절 변형(cabbage_spring, radish_spring)에 동일 URL 자동 공유 처리
- 나머지 40개 품목 주석 플레이스홀더 추가 (XXXXXX → 추후 등록)

**공정위 심사지침 준수**
- 고지 문구를 쿠팡 버튼 **위**에 오렌지색으로 명시 (기존 버튼 아래 → 가이드 위반 수정)
- 문구: "이 페이지는 쿠팡 파트너스 활동의 일환으로, 이에 따른 일정액의 수수료를 제공받습니다."
- 버튼 아래 모호한 "파트너스 링크 포함" 문구 제거

### 2026-05-05 — SEO + GA4 + 쿠팡 단독 링크 + 빌드 안정화

**SEO 최적화**
- meta description, Open Graph, Twitter Card 태그 추가 (기본 + 품목별)
- 품목 상세 title SEO 최적화 ("사과 가격 얼마니? — 오늘 사과 시세")
- `/sitemap.xml` 동적 엔드포인트 (64종 품목 포함)
- `/robots.txt` 엔드포인트 (sitemap 링크 포함)
- GA4 측정 ID env var(`GOOGLE_ANALYTICS_ID`) 지원 구조 완료 — ID 등록 대기

**쿠팡 단독 링크 정리**
- 네이버쇼핑 버튼 제거 (수익 없음, 정체성 희석 방지)
- 쿠팡 버튼 전체 너비 단독 배치

### 2026-05-05 — Sprint 2 쇼핑 링크 + 빌드 안정화

**쇼핑 링크 (Sprint 2 Part 1)**
- 품목 상세 페이지에 쿠팡·네이버쇼핑 외부 링크 버튼 추가
- `items.coupang_url` 컬럼 추가 (Alembic 마이그레이션) — 파트너스 링크 있으면 우선 사용, 없으면 검색 URL 폴백
- `scripts/set_coupang_urls.py` — 품목별 파트너스 URL 수동 등록 스크립트
- `urlencode_str` Jinja2 커스텀 필터 추가 (한글 품목명 URL 인코딩)

**빌드 안정화**
- Dockerfile `uv:latest` → `uv:0.11.7` 버전 고정 (Railway ghcr.io 타임아웃 방지)
